...
var loadSave = async function() {
  ...
  for (let i = 0x134; i <= 0x143; ++i) {
    title += String.fromCharCode(window.memory[i]);
  }
  ...
  console.log(title, window.game);
  for (let i = 0; i < 131072; ++i) {
    window.cartRam[i] = window.game.ram[i];
  }
}

var loadCart = async function(arr) {
  let buffer = new Uint8Array(arr);
  window.Module['FS_createDataFile']("/", "game.gb", buffer, true, true);
  let j = 0;
  for (let i = 0; i < 0x10000; ++i, ++j) {
    window.memory[j] = buffer[i];
  }
  j = 0;
  for (let i = 0; i < arr.byteLength; ++i, ++j) {
    window.cartRom[j] = buffer[i];
  }
  console.log(arr);
  await loadSave();
  Module._startCpu();
...
let write_memory = function(address, byte) {
  if (address >= 0xff10 && address <= 0xff3f) {
    window.audio.write(address, byte);
  }
}

let update_sound = function(cycles) {
  window.audio.updateCycles(cycles);
}
...
let draw = function() {
  ...
  window.smallCtx.putImageData(frames.shift(), 0, 0);
  window.mainCtx.imageSmoothingEnabled = false;
  window.mainCtx.drawImage(window.smallCanvas, 0, 0, 160, 144, 0, 0, CANVAS_WIDTH, CANVAS_HEIGHT);
...
let fn = function() {
  window.running = true;
  setInterval(() => {
    if (!window.audio.channel1.sources.length && !window.audio.channel1.currentlyPlaying.length) {
      frames.push(getImageData());
    } else if (frames.length == 0 && window.audio.channel1.currentlyPlaying.length <= 2) {
...
Module.onRuntimeInitialized = function() {
  window.memory = new Uint8Array(Module.HEAPU8.buffer, Module._getMemoryPtr(), 0x10000);
  window.cartRom = new Uint8Array(Module.HEAPU8.buffer, Module._getCartPtr(), 4194304);
  window.cartRam = new Uint8Array(Module.HEAPU8.buffer, Module._getRamPtr(), 131072);
};
...
var initialiseCanvas = function() {
  window.mainCanvas = document.getElementsByClassName("canvas")[0];
  window.mainCanvas.width = CANVAS_WIDTH;
  window.mainCanvas.height = CANVAS_HEIGHT;
  window.smallCanvas = document.createElement("canvas");
  window.smallCanvas.width = 160;
  window.smallCanvas.height = 144;
  window.mainCtx = window.mainCanvas.getContext("2d");
  window.smallCtx = window.smallCanvas.getContext("2d");
}
